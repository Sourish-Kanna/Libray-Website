...
const useNewsStore = create((set) => ({
  newsItems: [], // Initial state
    ...
    addNews: async (title) => {
        try {
        const response = await axios.post("https://libray-website-server.onrender.com/api/v1/news", {
            title,
        });
        if (response.status === 201) {
            set((state) => ({ newsItems: [...state.newsItems, response.data.news] }));
        } else {
    ...
    deleteNews: async (id) => {
        ...
        const response = await axios.delete(
            `https://libray-website-server.onrender.com/api/v1/news/${id}`
        );
    ...
    fetchNews: async () => {
        try {
        const response = await axios.get("https://libray-website-server.onrender.com/api/v1/news");
        if (response.status === 200) {
